...
// Binary search used for question 3
...
void mp_question3(String input, String territories[], double life_expectancy_factors[][15], int MAX_ROW, double temp_array[][15], String switch_factors[], int *index, int *result3) {
    ...
    String factors[15] = {"Base Life Expectancy", "Air pollution", "Ambient PM", "Ozone", "Household Air Pollution", "Environmental/Occupational Hazards", 
    ...
    Make sure to not include the Base Life Expectancy when transfering the values to tempArray*/
    for(i = 0; i < MAX_ROW; i++) {
    ...
    /*Assigns factors in switchfactors in order to sort it with the values later*/
    for(i = 0; i < 15; i++) {
    ...
    /*Binary Search for temp_string in order to get the index where the country is
    located in tempString array after sorting it*/
...
double mp_question4(String input, String territories[], double life_expectancy_factors[][15], int MAX_ROW, double life_expectancy, int *index_BE, int *result4) {
    int i, index;
    ...
    *index_BE = i; //assigns what index the territory is in the array
    ...
    */
    ...
    for(i = 0; i < NUMBER_OF_CASES; i++){
        ...
        if(result3 == 1) {
            ...
            printf("\nTerritory name input is invalid\n");
            break;
        }
    }
    ...
    for(i = 0; i < NUMBER_OF_CASES; i++){
        printf("\nTerritory: %s\n", q4_input[i]);
        life_expectancy = mp_question4(q4_input[i], territories, life_expectancy_factors, MAX_ROW, life_expectancy, &index_BE, &result4);
        if(result4 == 1) {
            ...
            printf("\nTerritory name could not be found\n");
        }
	}
